...
export const AdminController = {
    ...
    async updateUserRoles(req, res) {
        try {
            ...
            const updatedUser = await AdminService.updateUser(Number(userId), roles);
     ...
     */
    async moderateTrip(req, res) {
        try {
            roleCheck(['admin'])(req, res, () => { });
            const { tripId } = req.params;
            const { status, reason } = req.body;
            const result = await AdminService.moderateTrip(Number(tripId), status);
            ApiResponse.sendSuccess(res, result);
        }
        catch (error) {
            ApiResponse.sendError(res, error instanceof Error ? error : new Error('Unknown error'));
    ...
    async getAnalytics(req, res) {
        try {
            ...
            const analytics = await AdminService.generateSystemAnalytics(period?.toString() || '30d');
    ...
    async updateSystemConfig(req, res) {
        try {
            ...
            const result = await AdminService.updateSystemConfig(configKey, configValue);
            ApiResponse.sendSuccess(res, result);
        }
        catch (error) {
            ApiResponse.sendError(res, error instanceof Error ? error : new Error('Unknown error'));
