...
const { User, Trip, Review, Subscription, Follower } = db
...
async function seedDatabase() {
  try {
    ...
    await Review.create({
      ...
      user_id: adminUser.id,
      rating: 5,
      content: 'The Rocky Mountain experience was unforgettable! Perfect mix of challenging terrain and scenic beauty.'
    })
    ...
    await Review.create({
      ...
      user_id: premiumUser.id,
      rating: 4.5,
      content: 'The desert landscape was mesmerizing. Great roads and even better company!'
    })
    ...
    await Review.create({
      ...
      user_id: nonPremiumUser.id,
      rating: 4,
      content: 'Southwest charm at its best. The historic towns were a highlight of the journey.'
    })

    // Create follower relationships for social feed
    await Follower.create({
      follower_id: nonPremiumUser.id,
      following_id: premiumUser.id
    })

    await Follower.create({
      follower_id: premiumUser.id,
      following_id: adminUser.id
    })

    await Follower.create({
      follower_id: adminUser.id,
      following_id: nonPremiumUser.id
    })
