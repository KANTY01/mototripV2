...
class BookingService {
  async createBooking(bookingData: any, userId: number) {
    return db.transaction(async (t: Transaction) => {
      ...
      await Trip.update(
        { seats_available: trip.seats_available - bookingData.seats_requested },
        { where: { id: bookingData.trip_id }, transaction: t }
      );
  ...
  async getBookings(filters: BookingFilters) {
    ...
    if (filters.tripId) whereClause.trip_id = filters.tripId;
  ...
  async deleteBooking(id: number) {
    ...
    return db.transaction(async (t: Transaction) => {
      ...
      if (trip) {
        await Trip.update(
          { seats_available: trip.seats_available + booking.seats_requested },
          { where: { id: booking.trip_id }, transaction: t }
        );
      }
