...
router.get('/', async (req, res) => {
  try {
    ...
    await redisClient.set(cacheKey, JSON.stringify(trips), 'EX', 3600) // Cache for 1 hour
...
router.get('/:id', async (req, res) => {
  ...
  try {
    ...
    await redisClient.set(cacheKey, JSON.stringify(trip), 'EX', 3600) // Cache for 1 hour
